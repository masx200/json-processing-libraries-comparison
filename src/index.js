...
class BlogApp {
    ...
    extractMetadata(html, filename) {
        ...
        // 如果没有找到合适的文本，使用默认摘要
        if (!excerpt) {
            excerpt = `这是一篇关于 ${this.generateTitleFromFilename(filename)} 的技术文章，包含详细的分析和研究内容。`;
        }
    ...
    createArticleCard(article) {
        ...
        card.className = 'article-card bg-white rounded-lg shadow-lg overflow-hidden cursor-pointer hover:shadow-xl';
        ...
        categoriesContainer.innerHTML = categories.map(category => `
            <button class="category-tag px-4 py-2 rounded-full transition ${
                ...
                <span class="ml-2 text-sm">(${
                    category === 'all' ? this.articles.length : this.articles.filter(a => a.category === category).length
                })</span>
            </button>
        `).join('');
